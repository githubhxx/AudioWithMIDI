import os
import numpy as np
from .deprecated.sequence import EventSeq, ControlSeq # 加个.代表当前目录，即midi目录
import torch
...
def find_files_by_extensions(root, exts=[]):
    def _has_ext(name):
        if not exts: # exts代表文件扩展名列表，如果为空，则返回True。if not用于判断列表是否为空，为空则执行代码块
            return True
        name = name.lower()
        for ext in exts:
            if name.endswith(ext):
                return True
        return False
    for path, _, files in os.walk(root): # os.walk() 是一个 Python 标准库函数，用于遍历目录树。它返回一个生成器，每次迭代返回一个三元组 (path, dirs, files)，分别表示当前目录路径、子目录列表和文件列表。
        for name in files:
            if _has_ext(name): # _has_ext() 函数用于检查文件名是否以指定的扩展名结尾。如果文件名以指定的扩展名结尾，则返回True，否则返回False。
                yield os.path.join(path, name) # 返回一个生成器，每次迭代返回一个文件路径。
...
def get_masked_with_pad_tensor(size, src, trg, pad_token):
    ...
    src = src[:, None, None, :]
    trg = trg[:, None, None, :]
    src_pad_tensor = torch.ones_like(src).to(src.device.type) * pad_token # torch.ones_like(src) 创建一个与src形状相同的全1张量
    src_mask = torch.equal(src, src_pad_tensor) # torch.equal(src, src_pad_tensor) 判断src和src_pad_tensor是否相等
    trg_mask = torch.equal(src, src_pad_tensor)
    if trg is not None:
        trg_pad_tensor = torch.ones_like(trg).to(trg.device.type) * pad_token
        dec_trg_mask = trg == trg_pad_tensor # trg == trg_pad_tensor 判断trg和trg_pad_tensor是否相等
        ...
        look_ahead_mask = dec_trg_mask | seq_mask # | 表示按位或，即dec_trg_mask和seq_mask中只要有一个为True，look_ahead_mask就为True
...
def sequence_mask(length, max_length=None):
    ...
    x = torch.arange(max_length, dtype=length.dtype, device=length.device) # torch.arange(start, end, step=1) 创建一个从start到end的等差数列，步长为step
    return x.unsqueeze(0) < length.unsqueeze(1) # unsqueeze(0) 在第0维上增加一个维度，unsqueeze(1) 在第1维上增加一个维度
