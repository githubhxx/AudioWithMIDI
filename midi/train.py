...
from .custom.criterion import SmoothCrossEntropyLoss, CustomSchedule # 交叉熵损失和自定义学习率调度器
...
if __name__ == '__main__':
    # set config
    parser = custom.get_argument_parser() # -m 和 -c 参数解析器
    args = parser.parse_args()
    config.load(args.model_dir, args.configs, initialize=True) # super.load(config),同时初始化self._set_vocab_params()
    ...
    # load data
    dataset = Data(config.pickle_dir) # 初始化读取所有pickle文件，并划分train eval test数据集
    print(dataset) # 会自动调用__repr__方法
    ...
    mt = MusicTransformer(
                ...
                vocab_size=config.vocab_size, # config.load()已初始化，self.vocab_size = self.event_dim + 3
