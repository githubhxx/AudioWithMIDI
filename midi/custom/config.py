...
class Config:
    ...
    def __setattr__(self, key, value): # 重写python内置函数，由Python解释器自动调用
        try:
            # Use base implementation of __getattribute__ to check if this
            # attribute already exists on the class / instance (reserved key).
    ...
    def __getattr__(self, key): # 这个函数的作用是当一个属性不存在时，自动调用__getattribute__方法
        try:
            # Fallback: try to get a "normal" attribute first.
            return super().__getattribute__(key)
        except AttributeError:
            return self.dict[key]

    def __setitem__(self, key, value): # 这个函数用来设置字典的键值对，从而设置config的属性
        if key in self.dict and self.dict[key] != value:
            logging.info("'{}' is set to '{}' instead of '{}'.".format(
                key, self.dict[key], value))
        self.dict[key] = value

    def __getitem__(self, key): # 获取config属性
        return self.dict[key]
...
class MusicTransformerConfig(Config):
    def load(self, model_dir, configs, initialize=False, print=True):
        super().load(model_dir, configs, initialize, print) # 调用父类Config的load方法(获取save.yml文件中的配置)
        self._set_vocab_params()

    def _set_vocab_params(self):
        self.pad_token = self.event_dim # 这四行代码会自动调用__getattr__方法和__setattr__方法
