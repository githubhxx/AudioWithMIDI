...
def encode_midi(file_path): # 读取一个 MIDI 文件，输出一个整数 ID 序列
    events = []
    notes = []
    mid = pretty_midi.PrettyMIDI(midi_file=file_path) # 读取一个 MIDI 文件，输出一个 pretty_midi.PrettyMIDI 对象
    ...
    for inst in mid.instruments:
        ...
        ctrls = _control_preprocess([ctrl for ctrl in inst.control_changes if ctrl.number == 64]) # 输入：一个乐器的控制事件（control changes），主要看 number=64（sustain pedal）输出：一组 `SustainDownManager`，即每一段 sustain 按下-抬起的区间
        notes += _note_preprocess(ctrls, inst_notes) # 输入：一组 sustain 管理器和一组音符，输出：一个按时间排序的音符列表，其中 sustain 按下期间的所有音符都被平移到按下时间 目标：在有 sustain 的情况下，调整各音符的结束时间（延音），并整理成统一时间流 

    dnotes = _divide_note(notes) # 输入：一个按时间排序的音符列表，输出：一个按时间排序的音符列表，其中每个音符都被拆分成两个事件（note_on 和 note_off）

    # print(dnotes)
    dnotes.sort(key=lambda x: x.time) # 输入：一个按时间排序的音符列表，输出：一个按时间排序的音符列表
    # print('sorted:')
    # print(dnotes)
    cur_time = 0
    cur_vel = 0
    for snote in dnotes:
        events += _make_time_sift_events(prev_time=cur_time, post_time=snote.time) # 输入：两个时间点，输出：一系列时间偏移事件，确保时间间隔不超过 RANGE_TIME_SHIFT
        events += _snote2events(snote=snote, prev_vel=cur_vel) # 把单个 `SplitNote` 转成一串 `Event` 例如：输出一个类型为 `snote.type`（note_on/note_off）、value 为 pitch 的事件；返回：`[velocity_event?, note_on/off_event]`
        # events += _make_time_sift_events(prev_time=cur_time, post_time=snote.time)

        cur_time = snote.time
        cur_vel = snote.velocity

    return [e.to_int() for e in events]  # 把所有 `Event` 映射成整数 ID
