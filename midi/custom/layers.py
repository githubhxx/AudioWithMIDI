...
# 优先导入本项目的 midi.utils，而不是环境中的其他 utils 包
try:
    from .. import utils
    from ..utils import sequence_mask
except ImportError:
    # 回退到绝对导入，兼容直接从项目根目录运行脚本的情况
    from midi import utils
    from midi.utils import sequence_mask
...
class DynamicPositionEmbedding(torch.nn.Module):
    def __init__(self, embedding_dim, max_seq=2048):
        ...
        embed_sinusoid_list = np.array([[
            ...
            for pos in range(max_seq) # for会循环执行前边的表达式
        ]])
...
class RelativeGlobalAttention(torch.nn.Module):
    ...
    def __init__(self, h=4, d=256, add_emb=False, max_seq=2048, **kwargs):
        ...
        self.E = torch.randn([self.max_seq, int(self.dh)], requires_grad=False) # randn(size, mean=0, std=1) 创建一个形状为size，均值为0，标准差为1的正态分布随机数
        if self.additional:
            self.Radd = None
    ...
    def forward(self, inputs, mask=None, **kwargs):
        ...
        k = k.permute(0, 2, 1, 3) # permute(0, 2, 1, 3) 将第0维和第2维交换，第1维和第3维交换
        ...
        QE = torch.einsum('bhld,md->bhlm', [q, E]) # einsum(equation, *operands) 根据equation计算operands的乘积
    ...
    def _skewing(self, tensor: torch.Tensor): # 将tensor进行padding，然后 reshape，然后返回Srel
...
class Encoder(torch.nn.Module):
    def __init__(self, num_layers, d_model, input_vocab_size, rate=0.1, max_len=None):
        ...
        if True:
            self.pos_encoding = DynamicPositionEmbedding(self.d_model, max_seq=max_len) # 添加正弦位置编码
