...
class Data:
    def __init__(self, dir_path):
        self.files = list(utils.find_files_by_extensions(dir_path, ['.pickle'])) # 所有mid文件路径的列表
        self.file_dict = {
            'train': self.files[:int(len(self.files) * 0.8)], # :int(len(self.files) * 0.8)这里表示取前80%的文件
    ...
    def __repr__(self): # 重写python内置函数，由Python解释器自动调用
        return '<class Data has "'+str(len(self.files))+'" files>' # 返回一个字符串，表示数据集的文件数量
    ...
    def slide_seq2seq_batch(self, batch_size, length, mode='train'):
        ...
        x = data[:, :-1] # 取data的第0维，去掉最后一列
        y = data[:, 1:] # 取data的第0维，去掉第一列
        return x, y
    ...
    def _get_seq(self, fname, max_length=None): 
        ...
        if max_length is not None:
            if max_length <= len(data):
                start = random.randrange(0,len(data) - max_length) # 随机生成一个0到len(data) - max_length之间的整数
